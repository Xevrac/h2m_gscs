...
handle_bot_count()
{
    ...
    bot_quota = getDvarInt("botQuota"); // Get desired bot quota

    level endon("game_ended");

    // Server bot handling loop
    for (;;)
    {
        wait 0.5;
        player_count = getPlayerCount();
        current_bot_count = contBots();
        total_entities = player_count + current_bot_count;

        if (total_entities < bot_quota)
        {
            fill_amount = bot_quota - total_entities;
            spawnBots(fill_amount);
            iprintln("[BOT] Filling with " + fill_amount + " bots to reach quota " + bot_quota);
        }
        else if (total_entities > bot_quota)
        {
            kickbot(); 
        }
...
// Func to count human players
getPlayerCount()
{
    player_count = 0;
    foreach (player in level.players)
    {
        if (!isentityabot(player)) // Count only human players
        {
            player_count++;
        }
    }
...
spawnBots(amount)
{
    for (i = 0; i < amount; i++)
    {
        executecommand("spawnbot autoassign");
        wait 0.1;
    }
}
